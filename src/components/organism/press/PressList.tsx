...
const PressList = () => {
    ...
    useEffect(() => {
        const run = async () => {
            ...
            if (items.length > 0) {
                // Ada artikel — animate press items
                gsap.fromTo(
                    items,
                    { opacity: 0, y: 24 },
                    { opacity: 1, y: 0, duration: 0.6, ease: "power3.out", stagger: 0.08, delay: 0.05 }
                ...
                // Tidak ada artikel sama sekali (pressData kosong) — animate empty state
    ...
    const handleFilter = async (newFilter: string) => {
        ...
        // Cek apakah filter baru akan menghasilkan 0 artikel
        const nextFilteredCount =
            newFilter === "all"
                ? pressData.length
                : pressData.filter((p) => slugify(p.category) === newFilter).length;
        ...
        // Juga fade out empty state jika sedang tampil
        const currentEmpty = emptyRef.current;

        // Kill semua animasi yang sedang berjalan
        gsap.killTweensOf(allItems);
        if (currentEmpty) gsap.killTweensOf(currentEmpty);

        const tl = gsap.timeline();

        // Fade out empty state jika ada
        if (currentEmpty && parseFloat(getComputedStyle(currentEmpty).opacity) > 0) {
            tl.to(currentEmpty, { opacity: 0, y: -10, duration: 0.25, ease: "power2.in" }, 0);
        }
        ...
        tl.call(
            () => {
                if (nextFilteredCount === 0) {
                    // Tidak ada artikel — tampilkan empty state
                    const emptyEl = emptyRef.current;
                    if (emptyEl) {
                        gsap.fromTo(
                            emptyEl,
    ...
    // Filtered articles untuk di-render
    const filteredArticles = useMemo(() => {
        if (activeFilter === "all") return pressData;
        return pressData.filter((p) => slugify(p.category) === activeFilter);
    }, [activeFilter]);

    // Kondisi: tidak ada artikel sama sekali di pressData (bukan karena filter)
    const isGloballyEmpty = pressData.length === 0;
    ...
    return (
        <section className="pt-35 lg:pt-50 pb-25 px-9 xl:px-21 bg-primary border-b border-foreground/30 md:border-0">
            ...
            <div className="flex flex-col gap-25 lg:gap-50">
                {/* Filters — sembunyikan jika tidak ada artikel sama sekali */}
                {!isGloballyEmpty && (
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4 lg:gap-8">
                        <div className="md:col-span-3 flex flex-row gap-4 items-center">
                            <div className="w-3 h-3 bg-foreground"></div>
                        ...
                        <div className="md:col-span-8 md:col-start-5 flex flex-wrap gap-4 lg:gap-7.5 md:justify-end">
                            ...
                            {categories.map((category) => {
                                ...
                                return (
                                    <button
                                        key={slug}
                                        className={`cursor-pointer p-2.5 px-5 flex items-center capitalize gap-2.5 text-base transition-all duration-500 ease-in-out shrink-0 w-fit ${
                                            activeFilter === slug
                                                ? "text-[#1A1A19] bg-[#F8FAFC]"
                                                : "text-[#F8FAFC] border border-[#F8FAFC] hover:bg-[#F8FAFC] hover:text-[#1A1A19]"
                                        }`}
                ...
                {/* Press */}
                <div className="flex flex-col gap-8 lg:gap-15">
                    {/* Showing counter — sembunyikan jika tidak ada artikel sama sekali */}
                    {!isGloballyEmpty && (
                        <span className="text-base lg:text-xl xl:text-2xl text-foreground">
                            Showing{" "}
                            {activeFilter === "all"
                    ...
                    >
                        {filteredArticles.length === 0 ? (
                            <div
                                ref={emptyRef}
                                style={{ opacity: 0 }}
                                className="col-span-12 flex flex-col items-center justify-center py-25 lg:py-40 gap-6"
                            >
                                {/* Text */}
                                <div className="flex flex-col items-center gap-2.5 text-center">
                                    <span className="text-xl lg:text-2xl xl:text-3xl font-medium text-foreground">
                                        No articles found
                                    </span>
                                    <span className="text-sm lg:text-base text-foreground/45 max-w-xs">
                                        {isGloballyEmpty
                                            ? "There are no articles available yet."
                                            : "There are no articles under this category yet."}
                                    </span>
                                </div>
                            </div>
                        ) : (
                                ...
                                return (
                                    ...
                                    >
                                        <TransitionLink
                                            ...
                                            className="group flex flex-row gap-4 lg:gap-8"
                                        >
                                            <div className="relative w-[300px] lg:w-[400px]">
                                                <Image
                                                    src={article.pressThumbnail}
